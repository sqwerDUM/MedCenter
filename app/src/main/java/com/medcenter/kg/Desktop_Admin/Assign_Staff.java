...
public class Assign_Staff extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (y.moveToFirst()) {
            while (true) {
                if ((y.getString(7)).equals("Доктор")) {
        ...
        if (y1.moveToFirst()) {
            while (true) {
                if (y1.getString(7).equals("Сотрудник")) {
        ...
        b_assign.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (z.moveToFirst()) {
                    Message.message(Assign_Staff.this, "Пара уже назначена");
                } else {
                    boolean x = dbh.insert_staff(s_u.get(id_s), s_p.get(id_s), d_u.get(id_d), d_p.get(id_d), "Y");

                    if (x) {
                        Message.message(Assign_Staff.this, "Назначенный персонал");
                    } else {
                        Message.message(Assign_Staff.this, "Произошла ошибка");
                    }
                }
            }
        });
